...
final class NetworkScanBridge: @unchecked Sendable {
  ...
  func scan() {
    ...
    queue.async { [weak self] in
      ...
      guard let (local, mask) = self.wifiAddress() else {
        DispatchQueue.main.async { self.fireComplete(gen: gen) }
        return
      }

      let hosts = self.subnetHosts(address: local, mask: mask)
      let batchSize = 50
  ...
  private func wifiAddress() -> (String, String)? {
    ...
    guard let local = address, let netmask = mask else { return nil }
    return (local, netmask)
  }

  private func subnetHosts(address: String, mask: String) -> [String] {
    let ipParts = address.split(separator: ".").compactMap { UInt32($0) }
